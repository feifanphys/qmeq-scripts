...
omegapres, omegaflip = 0.00*U1, 0.00*U1
...
def stab_calc(system, bfield, vlst, vglst, dV=0.0001):
    ...
    for j1 in range(vgpnt):
        ...
        for j2 in range(vpnt):
            ...
            stab_cond[j1, j2] = abs(stab[j1, j2]) + 0.00001
            stab_cond[j1, j2] = np.log(stab_cond[j1, j2])
            #stab[j1,j2] = mu1-mu2
            #
...
vpnt, vgpnt = 201, 201
vlst = np.linspace(-800, 800, vpnt)
vglst = np.linspace(-400, 1600, vgpnt)
...
def stab_plot(stab, stab_cond, vlst, vglst, gam):
    ...
    p2.set_ylabel('$V_{g2}$', fontsize=20);
    ...
    cbar2.set_label('Log Current', fontsize=20)

    tt = 'Jt_Vbias = ' + str(vbias) + 'meV Je = ' + str(Je) + ' Jt1 = ' + str(Jt1) + ' Jt2 = ' + str(Jt2)
    plt.title(tt, loc='center')
    #
    plt.tight_layout()
    plt.savefig("../images/monte_carlo/" + tt + ".png")
    #plt.show()
