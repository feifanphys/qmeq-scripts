...
mulst = {0: -vbiasL, 1: -vbiasR, 2: -vbiasL, 3: -vbiasR,  4: -vbiasL, 5: -vbiasR}
tlst =  {0: temp,    1: temp,     2: temp,    3: temp,  4: temp, 5: temp}
...
mat1 = np.zeros((nstate,nstate),dtype=np.complex_)
mat1[4,4] = 1
mat1[6,6] = 1
mat1[7,7] = 1

mat2 = np.zeros((nstate,nstate),dtype=np.complex_)
mat2[2,2] = 1
mat2[3,3] = 1
mat2[7,7] = 1

mat3 = np.zeros((nstate,nstate),dtype=np.complex_)
mat3[1,1] = 1
mat3[3,3] = 1
mat3[7,7] = 1
...
def stab_calc(system, bfield, vlst, vglst, dV=0.0001):
    ...
    for j1 in range(vgpnt):
        ...
        for j2 in range(vpnt):
            ...
            eig_C = np.conjugate(eigenstate_in_fock)
            dm_fock = np.matmul(eig_T, np.matmul(dm_eigen, eig_C))
            product = np.matmul(dm_fock,mat3)
            stab[j1, j2] = abs(np.trace(product))
