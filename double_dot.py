...
# 0 is up, 1 is down
hsingle = {(0, 0): -vgate+interval,
           (1, 1): -vgate,
           (2, 2): -vgate+offset+interval,
           (3, 3): -vgate+offset,
           (0, 2): mu,
           (1, 3): mu,
           (0, 3): mu/5,
...
def stab_calc(system, bfield, vlst, vglst, dV=0.0001):
    ...
    for j1 in range(vgpnt):
        system.change(hsingle={(0,0):-0.9*vglst[j1]+interval,
                              (1,1):-0.9*vglst[j1],
                              (2,2):-0.8*vglst[j1]+offset+interval,
                              (3,3):-0.8*vglst[j1]+offset,
                              (0, 2): mu,
                              (1, 3): mu,
                              (0, 3): mu/5,
                              (1, 2): mu/5})
