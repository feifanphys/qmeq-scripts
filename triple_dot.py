...
def stab_calc(system, bfield, vlst_1, vlst_2, dV=0.0001):
    vpnt, vgpnt = vlst_1.shape[0], vlst_2.shape[0]
    stab = np.zeros((vpnt, vgpnt))
    stab_cond = np.zeros((vpnt, vgpnt))
    #
    for j1 in range(vgpnt):      
        for j2 in range(vpnt):
            system.change(hsingle={(0,0):-alpha_1*vlst_1[j1]-beta_1*vlst_2[j2]+offset,
                              (1,1):-alpha_2*vlst_1[j1]-beta_2*vlst_2[j2] + 0.6*offset,
                              (2,2): -0.5*vlst_1[j1]-0.5*vlst_2[j2],
                              (0,1): tc,
                              (1,2): tc})
...
vgpts_1, vgpts_2 = 201, 201
vglst_1 = np.linspace(-15*U, 15*U, vgpts_1)
vglst_2 = np.linspace(-15*U, 15*U, vgpts_2)
stab, stab_cond = stab_calc(system, bfield, vglst_1, vglst_2)
...
def stab_plot(stab, stab_cond, vlst, vglst, U, gam):
    (xmin, xmax, ymin, ymax) = np.array([vglst_1[0], vglst_1[-1], vglst_2[0], vglst_2[-1]])/U
    fig = plt.figure(figsize=(12,4.2))
    #
    p1 = plt.subplot(1, 2, 1)
    p1.set_xlabel('$V_{g1}/U$', fontsize=20)
    p1.set_ylabel('$V_{g2}/U$', fontsize=20)
    ...
    p2.set_xlabel('$V_{g1}/U$', fontsize=20);
    p2.set_ylabel('$V_{g2}/U$', fontsize=20);
...
stab_plot(stab, stab_cond, vglst_1, vglst_2, U, gam)